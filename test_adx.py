...
import numpy as np
import pandas as pd
from warrant_app.utils.trading_util import calc_di_adx
...
pdi14_array,ndi14_array, adx_array=calc_di_adx(highest_array, lowest_array, closing_array, SMOOTHING_FACTOR=14)
for i in np.arange(adx_array.size):
    print "pdi=%s, ndi=%s, adx=%s" % (pdi14_array[i],ndi14_array[i],adx_array[i])