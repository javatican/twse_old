...
if not os.path.isfile(filename):
    ...
    for warrant in warrant_list:
        iv_list = warrant.twse_trading_warrant_list.filter(implied_volatility__isnull=False).values_list('implied_volatility', flat=True)
...
rect_sat = axarr[0].bar(x_pos[sat], iv_diff[sat], width, color='c')
