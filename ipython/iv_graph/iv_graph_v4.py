...
# This is the 4th version: only select warrants that have 'full' list of trading_warrant
# (ie. for every available trading_date) and also calculate std(standard deviations)
# and then filter by std.
import json
import matplotlib 
import os
...
if not os.path.isfile(filename):
    ...
    for warrant in warrant_list:
        ...
        if len(a_list) != trading_date_count: continue
    ...
    json_data = {'total_warrant_count':i,
                 'trading_date_list':list(trading_date_list),
                 'iv_data': iv_data}
...
for iv_list in iv_data:
    ...
    std=np.std(iv_array[bool_mask])
    #print std
    #filter by std
    if std>0.02: continue
    x=x_pos[bool_mask]
...
ax.set_ylabel('IV')
ax.set_title('Warrant IV graphs(total %s lines)' % i)
